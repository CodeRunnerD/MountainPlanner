...
import { canTransition, getEditableStatuses } from "#/lib/routeStatus";
...
function EditRoutePage() {
	...
	const [existingWaypoints, setExistingWaypoints] = useState<RouteWaypoint[]>(
		[],
	);
	...
	useEffect(() => {
		const fetchRoute = async () => {
			...
			if (r) {
				setRoute(r);
				setExistingWaypoints(w || []);
				const parsedWaypoints: ParsedWaypoint[] =
					w?.map((wp) => ({
						lat: wp.lat,
						lng: wp.lng,
	...
	// Load draft on mount (only if no server data conflict - but server data takes precedence)
	useEffect(() => {
		if (!loading && route) {
			const draft = loadDraft();
			if (draft && route.status === "draft" && isCreator) {
				// Optionally merge draft data if it's newer than server
	...
	const handleSubmit = async (
		...
		if (!route) return;
		if (!canTransition(route.status, targetStatus, role, isCreator)) {
			alert("No tienes permiso para realizar esta transición de estado");
			return;
		}
		...
		let gpxFilePath = route?.gpx_file_path;

		// Upload new file if present
		if (gpxFile) {
			const { path, error } = await uploadRouteGpx(gpxFile, routeId);
			if (error) {
		...
		return (
			<div className="text-center py-12 space-y-4">
				<p className="text-muted-foreground">
					No puedes editar esta ruta. Solo los organizadores pueden editar
					rutas publicadas.
				</p>
				<Button asChild variant="link">
	...
	return (
		<div className="space-y-6 max-w-4xl mx-auto">
			...
			onSubmit={(e) => {
				e.preventDefault();
				e.stopPropagation();
				// Keep current status on Enter; use DraftActions buttons for explicit transitions
				handleSubmit(route.status);
			}}
			>
					...
					<CardContent className="space-y-4">
						{route.gpx_file_path && (
							<p className="text-sm text-muted-foreground">
								Archivo actual: {route.gpx_file_path}
							</p>
						)}
						<div
							...
							onClick={() =>
								document.getElementById("gpx-upload-edit")?.click()
							}
						>
							<div className="text-center">
								...
								<p className="mt-2 text-sm font-medium">
									{gpxFile
										? gpxFile.name
										: "Arrastra un archivo GPX/KML o haz clic para reemplazar"}
								</p>
				...
				<DraftActions
					...
					onClearDraft={() => {
						...
						form.reset({
							...
							skills: existingWaypoints.map((x) => x.name) || [],
						});
						setWaypoints(
						...
						setTrackPoints(
							existingWaypoints.map((wp) => [wp.lat, wp.lng] as [number, number]),
						);
						setGpxFile(null);
					}}
				/>
